...
/// any vector adapter (full SQL over JSONB): it declares the complete operator set and emits the
/// <b>null-inclusive</b> forms for <c>Ne/Nin/HasNone</c> so its result-set matches the convergence
/// oracle's locked null semantics. It is fail-loud — the coordinator has already validated the tree
/// is fully pushable, so a node outside capabilities here is a backstop throw. The reader lowers
/// <c>$between</c> → <c>AllOf(Gte,Lte)</c> and wildcards → StartsWith/EndsWith/Contains, so this
/// translator carries no Like/Between arms.
/// </summary>
/// <remarks>
/// A static translator (no generic <c>IVectorFilterTranslator&lt;TNative&gt;</c> interface — that
/// surface was retired from Koan core; the live Qdrant connector follows the same shape). The repo
/// declares the capability via <see cref="Caps"/> and renders the WHERE via the static
/// <see cref="Translate(Filter?, DynamicParameters)"/>.
/// </remarks>
internal static class PGVectorFilterTranslator
{
    ...
    /// <summary>Build the WHERE fragment (no "WHERE" keyword) into an existing parameter bag.</summary>
