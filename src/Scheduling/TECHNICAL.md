# Agyo.Scheduling - Technical reference
...
- Reserved (Phase 2): Cron (`ICronScheduled`), Allowed windows (`IAllowedWindows`), Distributed lock (`IProvidesLock`). Interfaces exist; runtime gating will be added in a future revision. For the durable / cron / distributed-lock story today, use `Sylin.Koan.Jobs` (the ledger-backed tier).
