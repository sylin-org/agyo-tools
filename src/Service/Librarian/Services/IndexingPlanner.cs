...
public sealed class IndexingPlanner
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        foreach (var file in discoveredFiles)
        {
            ...
            if (forceReindex)
            {
                // Force re-index re-processes every discovered file, but a file is only "Changed"
                // (which clears its existing chunks first) when it was already indexed. A file with
                // no manifest entry is New — there is nothing to remove, so don't route it through
                // the changed path (which would look up a manifest entry that doesn't exist).
                if (manifest.ContainsKey(file.RelativePath))
