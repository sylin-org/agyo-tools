...
public class MetricsController : ControllerBase
{
    ...
    public async Task<IActionResult> GetHealth()
    {
        try
        {
            ...
            var projects = await Agyo.Service.Librarian.Models.Project.Query(p => true);
            var hasProjects = projects.Any();

            // Check if any projects are in failed state
            var failedProjects = projects.Count(p => p.Status == Agyo.Service.Librarian.Models.IndexingStatus.Failed);
        ...
        try
        {
            ...
            return Ok(new
            {
                ...
                metadata = new
                {
                    ...
                    degradedComponents = metrics.Components.Count(c => c.Status != Agyo.Service.Librarian.Services.HealthStatus.Healthy)
                }
            });
        }
        catch (Exception ex)
    ...
    public async Task<IActionResult> GetDashboardOverview(CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                var degraded = components.Components.Where(c => c.Status != Agyo.Service.Librarian.Services.HealthStatus.Healthy);
                foreach (var component in degraded)
                {
                    var severity = component.Status == Agyo.Service.Librarian.Services.HealthStatus.Critical ? "critical" : "warning";
            ...
            return Ok(new
            {
                data = new
                {
                    ...
                    componentHealth = new
                    {
                        ...
                        degradedCount = components.Components.Count(c => c.Status != Agyo.Service.Librarian.Services.HealthStatus.Healthy),
