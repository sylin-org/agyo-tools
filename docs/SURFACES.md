...
| Agyo.Service.Librarian | Agyo.Service.Librarian.Tests (ARCH-0079) | 2026-06-14 | 1 boot-smoke green + 1 infra-gated behavioral | Re-homed from Koan's Koan.Service.KoanContext (AGYO-0002); the first Agyo.Service.* runnable tool. Boot-smoke composes the WHOLE service via real AddKoan() (search/indexing pipeline + IndexProjectAsync delegate + file-watch hosted services + JobMaintenanceTask re-homed onto Agyo.Scheduling). Behavioral spec verified the ingest pipeline LIVE (Ollama all-minilm + per-project Weaviate class provisioning, 0 errors; fixed a force-reindex manifest bug). The search round-trip rides the bespoke async outbox and is pending the P4 Agyo.Rag re-platform — see AGYO-0002 §"Verified vs pending". |
